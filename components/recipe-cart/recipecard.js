    ...
    <div>
      ...
      <div className="p-4 flex flex-col bg-slate-900 text-red-100 justify-center">
        <div className="p-4 flex flex-col bg-slate-900 text-red-100">
          <Carousel showArrows={true}>
            {recipe.images.map((image) => (
              <div className="max-h-96" key={image}>
                <img src={image} alt={recipe.title} className="" />
              </div>
            ))}
          </Carousel>
        </div>
        <h2 className="text-3xl font-bold text-center p-5">{recipe.title}</h2>
          ...
          <div className="p-4">
            ...
            <ul>
              {Object.keys(recipe.ingredients).map((ingredientKey, index) => (
                <li key={index}>
                  {ingredientKey} : {recipe.ingredients[ingredientKey]}
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="flex space-x-4 justify-center gap-8">
          <p>
            <FontAwesomeIcon icon={faUtensils} size="xl" /> Prep: {convertToHours(recipe.prep)}
          </p>
          <p>
            <FontAwesomeIcon icon={faMitten} size="xl" /> Cook: {convertToHours(recipe.cook)}
          </p>
        </div>
