...
export default function SearchResultPage(props) {
  ...
  const searchResult = props.recipes; // Store the search result recipes received as a prop
  const query = router.query.searchQuery; // Extract the search query from the router's query parameters
...
export async function getServerSideProps(context) {
  const searchQuery = context.params?.searchQuery; // Extract the search query from the URL parameters
